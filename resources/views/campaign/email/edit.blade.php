        ...
        <div class="px-5 sm:px-20 mt-10 pt-10 border-t border-gray-200 dark:border-dark-5">
            ...
            <form action="{{ route('campaign.emails.update', $edit_campaign->id) }}" method="POST">
                ...
                <div class="intro-y box p-5">
                    ...
                    <div class="input-form mt-3"> 
                        ...
                        <select class="w-full form-select sm:w-1/2" name="smtp_server_id" required>
                            @forelse (getSmtpServerWiseList() as $provider => $email_provider)
                                <optgroup label="{{ Str::upper($provider) }} SERVER('s)">
                                    @forelse ($email_provider as $provider)
                                        <option value="{{ $provider->id }}" {{ $edit_campaign->smtp_server_id == $provider->id ? 'selected' : null }} class="normal-case">{{ Str::camel($provider->name) }}</option>
                                    @empty
                                                    ...
                                                    <div class="container pb-5">
                                                        <div class="row justify-content-center pb-5">
                                                            <div class="col-12 pb-5">
                                                                <input class="checkbox-tools" type="radio" value="{{ $templates->id }}" name="template_id" id="tool-{{ $templates->id }}" {{ $edit_campaign->template_id ==  $templates->id ? 'checked' : '' }}>
            ...
            <div class="intro-y inbox box mt-5">
                ...
                <div class="overflow-x-auto sm:overflow-x-visible myTable">
                    @forelse (campaingEmails() as $email)
                        <div class="intro-y">
                                ...
                                <div class="flex px-5 py-3">
                                    <div class="w-56 flex-none flex items-center mr-10">
                                        ...
                                        @foreach ($edit_campaign->campaign_emails as $campaign_email)
                                          ...
                                          @endforeach
                                        data-id="{{ $email->id }}"  data-email="{{ $email->email }}" name="emails[]" value="{{ $email->id }}" type="checkbox">
                                    ...
                                    <div class="grid grid-cols-3 w-full gap-4">
                                        <div class="w-64 sm:w-auto mr-10">
            ...
            <!-- END: Form Layout -->
                    ...
                    <div class="text-right mt-5">
                        <button type="submit" class="button w-40 bg-theme-1 text-white">@translate(Update Emails)</button>
                    </div>
